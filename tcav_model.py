...
class ModelWrapper(six.with_metaclass(ABCMeta, object)):
  ...
  def _find_ends_and_bottleneck_tensors(self, node_dict):
    print("find_ends_and_bottleneck_tensors")
  ...
  def _make_gradient_tensors(self):
    ...
    print("_make_gradient_tensors")
    #print("self.bottlenecks_tensors", self.bottlenecks_tensors)
    print("self.bottlenecks_tensors: ", len(self.bottlenecks_tensors))
    self.bottlenecks_gradients = {}
    for bn in self.bottlenecks_tensors:
      self.bottlenecks_gradients[bn] = tf.gradients(ys=self.loss, xs=self.bottlenecks_tensors[bn])[0]
      print("--- : ", tf.gradients(ys=self.loss, xs=self.bottlenecks_tensors[bn])[0])
  ...
  def run_examples(self, examples, bottleneck_name):
    ...
    """
    #print("def run_examples()")
    #print("self.bottlenecks_tensors[bottleneck_name]: ", self.bottlenecks_tensors[bottleneck_name])
    #print("{self.ends['input']: examples}: ", {self.ends['input']: examples})
...
class PublicImageModelWrapper(ImageModelWrapper):
  ...
  def __init__(self, sess, model_fn_path, labels_path, image_shape,
    ...
    with graph.as_default():
      ...
      print("self.y_input: ", self.y_input)
      print("self.ends['prediction']: ", self.ends['prediction'])
      self.pred = tf.expand_dims(self.ends['prediction'][0], 0)
      _labels = tf.one_hot(self.y_input, self.ends['prediction'].get_shape().as_list()[1])
...
class KerasModelWrapper(ModelWrapper):
  ...
  def __init__(
    ...
    self.y_input = tf.compat.v1.placeholder(tf.int64, shape=[None])
    print(self.model)
    print(self.model.loss_functions[0])
    print(self.model.loss_functions[0].fn)
    self.loss = self.model.loss_functions[0](self.y_input, self.ends['prediction'][0])
    print("self.model.loss_functions[0](self.y_input, self.model.outputs[0][0])", self.loss)
    self.loss = self.model.loss(self.y_input, self.ends['prediction'][0])
    print("self.loss calculation is over")
    print("self.loss : self.model.loss(self.y_input, self.model.outputs[0][0]) : ", self.loss)
    #self.loss = self.model.compiled_loss._losses[0].fn(self.y_input, self.model.outputs[0])
    self._make_gradient_tensors()
  ...
  def label_to_id(self, label):
    print("def label_to_id() ", self.labels.index(label))
    return self.labels.index(label)
  ...
  def import_keras_model(self, saved_path):
    ...
    self.model = tf.keras.models.load_model(saved_path, compile=True)
    print("self.model: ", self.model)
    print("type : ", type(self.model))
    print("self.model.get_loss_for: ", self.model.get_losses_for)
    print("self.model.loss: ", self.model.loss)
    print("self.model.optimizer: ", self.model.optimizer)
    print("self.model.predict: ", self.model.predict)
    print("self.model.outputs: ", self.model.outputs[0])
    #print("self.model.compiled_loss: ", self.model.compiled_loss)
    #print("self.model.compiled_loss._losses[0].fn: ", self.model.compiled_loss._losses[0].fn)
    print("***********************")
  ...
  def get_bottleneck_tensors(self):
    ...
    for layer in layers:
      ...
      if self.model_to_run == 'Inception':
        if 'mixed' not in layer.name:
          continue
        else:
          concat_layers.append((layer.name, layer.output))
      elif self.model_to_run == 'VGG':
        if 'pool' not in layer.name:
          continue
        else:
          concat_layers.append((layer.name, layer.output))
      elif self.model_to_run == 'DenseNet':
        if 'concat' not in layer.name:
          continue
        #if '2_conv' not in layer.name:
        #  continue
        else:
          concat_layers.append((layer.name, layer.output))
      elif self.model_to_run == 'Resnet':
        if 'out' not in layer.name:
          continue
        else:
          concat_layers.append((layer.name, layer.output))
      else:
    ...
    split = int(0.8 * len(concat_layers))
    concat_layers_cut = concat_layers[split:]
    #print(concat_layers_cut)
    for concat_layer in concat_layers_cut:
      self.bottlenecks_tensors[concat_layer[0]] = concat_layer[1]
  ...
  def get_image_shape(self):
    self.image_shape = list(self.model.input_shape)[1:]
    return self.image_shape