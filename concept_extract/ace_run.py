...
def main(args):
  ...
  masked_results_dir = os.path.join(args.working_dir, 'masked_results/')
  ...
  tf.io.gfile.makedirs(masked_results_dir)
  ...
  cd.create_patches(param_dict={'n_segments': [30]})
  ...
  print("finished: ace_helpers.save_concepts(cd, discovered_concepts_dir) ------------")
  # Calculating CAVs and TCAV scores
  cav_accuraciess = cd.cavs(min_acc=0.0)
  scores = cd.tcavs(test=False)
  print("finished: scores = cd.tcavs(test=False) ---------------")
  ace_helpers.save_ace_report(cd, cav_accuraciess, scores,
                                 results_summaries_dir + 'ace_results.txt')
  print("finished: ace_helpers.save_ace_report() ----------- ")
  ace_helpers.save_images_filename(cd, results_summaries_dir + 'raw_ace_filename_match.json')
  print("finished: ace_helpers.save_images_filename() ------------------ ")
  # Plot examples of discovered concepts
  for bn in cd.bottlenecks:
    ace_helpers.plot_concepts(cd, bn, 10, address=results_dir)
    print("finished: ace_helpers.plot_concepts() ------------------ ")
    ace_helpers.save_mask_plot(cd, bn, address = masked_results_dir, raw_img_address = results_dir, tcav_scores = scores)
    print("finished: ace_helpers.save_mask_plot() ------------------ ")
  # Delete concepts that don't pass statistical testing
  cd.test_and_remove_concepts(scores)
  del cd.patch_masks_dict
...
def parse_arguments(argv):
  ...
  parser.add_argument('--min_imgs', type=int,
      help="Minimum number of images in a discovered concept",
                      default=40)
  parser.add_argument('--num_parallel_runs', type=int,
