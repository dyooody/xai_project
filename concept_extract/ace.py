...
class ConceptDiscovery(object):
  ...
  def __init__(self,
    ...
    Args:
      ...
      working_dir: Name of the directory that conctains all the information (base directory)
  ...
  def create_patches(self, method='slic', discovery_images=None, raw_file_names=None, param_dict=None):
    print("def create_patches()")
    ...
    if param_dict is None:
      param_dict = {}
    dataset, image_numbers, patches, patch_masks = [], [], [], []
    patch_masks_dict = dict()
    #raw_imgs_dict = dict()
    if discovery_images is None:
      raw_imgs, raw_file_names = self.load_concept_imgs(
          self.target_class, self.num_discovery_imgs)
      self.discovery_images = raw_imgs
      self.discovery_images_filename = raw_file_names
    else:
      self.discovery_images = discovery_images
      self.discovery_images_filename = raw_file_names
    print("self.discovery_images : ", len(self.discovery_images))
    if self.num_workers:
      ...
      for fn, sp_outputs in enumerate(outputs):
        image_superpixels, image_patches, unique_masks = sp_outputs
        image_id = '0' * int(np.ceil(3 - np.log10(fn + 1))) + '{}'.format(fn + 1)
        patch_masks_dict[image_id] = unique_masks
        #raw_imgs_dict[image_id] = self.discovery_images_filename[fn]
        for superpixel, patch, mask in zip(image_superpixels, image_patches, unique_masks):
          dataset.append(superpixel)
          patches.append(patch)
          patch_masks.append(image_id+":"+str(mask[0]))
          image_numbers.append(fn)
    else:
      for fn, img in enumerate(self.discovery_images):
        image_superpixels, image_patches, unique_masks = self._return_superpixels(
        ...
        for superpixel, patch, mask in zip(image_superpixels, image_patches, unique_masks):
          ...
          patch_masks.append(image_id+":"+str(mask[0]))
          image_numbers.append(fn)
    self.dataset, self.image_numbers, self.patches, self.patch_masks =\
    np.array(dataset), np.array(image_numbers), np.array(patches), np.array(patch_masks)

    self.patch_masks_dict = patch_masks_dict
    #self.raw_imgs_dict = raw_imgs_dict
    print("self.dataset: ", self.dataset.shape)
    print("self.patches: ", self.patches.shape)
    print("self.patch_masks: ", self.patch_masks.shape)
    print("self.image_numbers: ", self.image_numbers.shape)

  def _return_superpixels(self, img, method='slic',
                          param_dict=None):
    #print("def _return_superpixels()")
    ...
    if param_dict is None:
      param_dict = {}
    if method == 'slic':
      #n_segmentss = param_dict.pop('n_segments', [15, 25, 35])
      n_segmentss = param_dict.pop('n_segments', [30])
    ...
    superpixels, patches, unique_patches = [], [], []
    up_idx = 0
    while unique_masks:
      superpixel, patch, unique_patch = self._extract_patch(img, unique_masks.pop())
      superpixels.append(superpixel)
      patches.append(patch)
      unique_patches.append([up_idx, unique_patch, -1, -1])
      up_idx += 1
    return superpixels, patches, unique_patches

  def _extract_patch(self, image, mask):
    #print("def _extract_path()")
    #print("image", image)
    #print("mask", mask)
  ...
  def _cluster(self, acts, method='KM', param_dict=None):
    ...
    centers = None
    if method == 'KM':
      n_clusters = param_dict.pop('n_clusters', 10)
  ...
  def discover_concepts(self,
    ...
    for bn in self.bottlenecks:
      ...
      if activations is None or bn not in activations.keys():
        print("self.dataset that goes to _patch_activations: ", self.dataset.shape)
        bn_activations = self._patch_activations(self.dataset, bn)
      else:
        bn_activations = activations[bn]
      print("bn_activations.shape: ", bn_activations.shape)
  ...
  def _random_concept_activations(self, bottleneck, random_concept):
    #print("def _random_concept_activations()")
    ...
    if not tf.io.gfile.exists(rnd_acts_path):
      rnd_imgs, rnd_imgs_filenames = self.load_concept_imgs(random_concept, self.max_imgs)
  ...
  def _concept_cavs(self, bn, concept, activations, randoms=None, ow=True):
    ...
    if randoms is None:
      ...
      #print("def _concept_cavs() - self.source_dir", self.source_dir)
      #print("def _concept_cavs() - self.target_class", self.target_class)
      randoms = [name for name in os.listdir(str(self.source_dir)) if os.path.isdir(os.path.join(str(self.source_dir), name))]
      randoms.remove(str(self.target_class))
      randoms.remove("random_discovery")
      #print("def _concept_cavs() - randoms: ", randoms)
    if self.num_workers:
      ...
      accs = []
      for rnd in randoms:
        cal_cav_result = self._calculate_cav(concept, rnd, bn, activations, ow)
        #print("self._calculate_cav(concept, rnd, bn, activations, ow): ", cal_cav_result)
        accs.append(cal_cav_result)
    return accs
  ...
  def cavs(self, min_acc=0., ow=True):
    ...
    concepts_to_delete = []
    for bn in self.bottlenecks:
      for concept in self.dic[bn]['concepts']: #concept: name of each concept e.g. Doberman_concept1
        concept_imgs = self.dic[bn][concept]['images']
        # patch with size of original image and they are in each concept length is same with max_imgs
        #print("=========================================== concept_imgs.shape : " + str(concept) + ":, " + str(concept_imgs.shape))
        # concepts_imgs.shape = (N, 224, 224, 3) e.g. (20, 224, 224, 3)
        concept_acts = get_acts_from_images(concept_imgs, self.model, bn)
        # concept_imgs after activation layer so the shape would be before classifier e.g. 7,7,512
        # concept_acts.shape = (N, 14, 14, 512) e.g. (20, 14, 14, 512)
        #print("concept_acts.shape: ", concept_acts.shape)
        acc[bn][concept] = self._concept_cavs(bn, concept, concept_acts, ow=ow)
        #print("acc[bn][concept].shape : ", len(acc[bn][concept]))
        # in _concept_cavs() they calculate the CAV score with other classses (excpet target score)
        # so the shape would be (N, 14, 14, 512) N = Number of classes - 1
  ...
  def _sort_concepts(self, scores):
    #print("def _sort_concepts()")
    for bn in self.bottlenecks:
      tcavs = []
      for concept in self.dic[bn]['concepts']:
        tcavs.append(np.mean(scores[bn][concept]["mean_tcav"]))
      concepts = []
  ...
  def _return_gradients(self, images):
    ...
    return gradients
  '''
  def _return_gradients(self, concept_acts):
    print("def _return_gradients()")
    ...
    for bn in self.bottlenecks:
      print("concept_acts.shape: ", concept_acts.shape)
      bn_grads = np.zeros((concept_acts.shape[0], np.prod(concept_acts.shape[1:])))

      for i in range(len(concept_acts)):
        bn_grads[i] = self.model.get_gradient(
            concept_acts[i:i+1], [class_id], bn, None).reshape(-1)
      gradients[bn] = bn_grads
    return gradients
  '''
  ...
  def _tcav_score(self, bn, concept, rnd, gradients):
    ...
    Returns:
      np.mean(prod <0) : TCAV score of the concept with respect to the given random counterpart
      prod2 : raw tcav score for all the images
    """
    vector = self.load_cav_direction(concept, rnd, bn)
    prod = np.sum(gradients[bn] * vector, -1)
    #concept_prod = np.sum(concept_gradients[bn] * vector, -1)

    prod_val = dict()
    prod_val["mean"] = np.mean(prod<0)
    #prod_val["raw"] = list(concept_prod)
    #print("len(prod_val['mean']: ", len(prod_val['mean']))
    #print("len(prod_val['raw']): ", len(prod_val['raw']))

    return prod_val

  '''def _get_concept_activation(self, bn, concept):
    print("def _get_concept_activation()")
    patch_masks = self.dic[bn][concept]['masks']
    img_id_list = []
    for i in range(len(patch_masks)):
      cur = patch_masks[i]
      img_id_list.append(cur.split(":")[0])
    print("img_id_list: ", img_id_list)
    #img_id_list = list(set(img_id_list))
    #print("img_id_list: ", img_id_list)
    filename_lists = []
    for iid in img_id_list:
      filename_lists.append(self.raw_imgs_dict[iid])

    concept_raw_imgs = load_images_from_files(
        filename_lists,
        max_imgs=2 * self.max_imgs,
        return_filenames=False,
        do_shuffle=False,
        run_parallel=(self.num_workers > 0),
        shape=(self.image_shape),
        num_workers=self.num_workers)
    tcav_score_images = concept_raw_imgs[-self.max_imgs:]
  ...
  def tcavs(self, test=False, sort=True, tcav_score_images=None):
    ...
    randoms.remove(str(self.target_class))
    randoms.remove("random_discovery")
    #print("def tcavs() - randoms: ", randoms)
    if tcav_score_images is None:  # Load target class images if not given
      raw_imgs, raw_imags_filenames = self.load_concept_imgs(self.target_class, 2 * self.max_imgs)
      tcav_score_images = raw_imgs[-self.max_imgs:]
    gradients = self._return_gradients(tcav_score_images)
    for bn in self.bottlenecks:
      for concept in self.dic[bn]['concepts'] + [self.random_concept]:
        tcav_scores[bn][concept] = dict()
        mean_tcav = []
        def t_func(rnd):
          return self._tcav_score(bn, concept, rnd, gradients)
        if self.num_workers:
          pool = multiprocessing.Pool(self.num_workers)
          tcav_scores[bn][concept]["mean_tcav"] = pool.map(lambda rnd: t_func(rnd)["mean"], randoms)
          #tcav_scores[bn][concept]["raw_tcav"] = pool.map(lambda rnd: t_func(rnd)["raw"], randoms)
        else:
          #tcav_scores[bn][concept]["raw_tcav"] = raw_tcav
          tcav_scores[bn][concept]["mean_tcav"] = [t_func(rnd)["mean"] for rnd in randoms]
          #tcav_scores[bn][concept]["raw_tcav"] = [t_func(rnd)["raw"] for rnd in randoms]
    if test:
      self.test_and_remove_concepts(tcav_scores)
    if sort:
      self._sort_concepts(tcav_scores)

    '''# get average of tcav_scores raw_tcav
    tcav_concepts = tcav_scores[bn].keys()
    for con in tcav_concepts:
      concept_data = tcav_scores[bn][con]
      raw_tcav_data = concept_data["raw_tcav"]
      np_raw_tcav = np.array(raw_tcav_data)
      avg_raw_tcav = np.mean(np_raw_tcav < 0, axis = 0)
      concept_data["avg_raw_tcav"] = list(avg_raw_tcav)
      del concept_data["raw_tcav"]'''

    with open(str(self.working_dir)+"/"+str(self.target_class)+"_"+str(self.model_name)+"_tcav_scores.json", "w") as outfile:
      json.dump(tcav_scores, outfile, indent=4)
    return tcav_scores

  def do_statistical_testings(self, i_ups_concept, i_ups_random):
    #print("def do_statistical_testings()")
  ...
  def test_and_remove_concepts(self, tcav_scores):
    #print("def test_and_remove_concepts()")
    ...
    for bn in self.bottlenecks:
      for concept in self.dic[bn]['concepts']:
        ...
        (tcav_scores[bn][concept]["mean_tcav"], tcav_scores[bn][self.random_concept]["mean_tcav"])
        if pvalue > 0.01:
          concepts_to_delete.append((bn, concept))
    for bn, concept in concepts_to_delete:
      self.delete_concept(bn, concept)
  ...
  def _concept_profile(self, bn, activations, concept, randoms):
    ...
    def t_func(rnd):
      #print("def t_func()")
      products = self.load_cav_direction(concept, rnd, bn, len(self.dic[bn][concept]['images'])) * activations
  ...
  def find_profile(self, bn, images, mean=True):
    ...
    #randoms = ['random500_{}'.format(i) for i in np.arange(self.num_random_exp)]
    print("def find_profile() - self.source_dir", self.source_dir)
    print("def find_profile() - self.target_class", self.target_class)
    randoms = [name for name in os.listdir(str(self.source_dir)) if os.path.isdir(os.path.join(str(self.source_dir), name))]
    randoms.remove(str(self.target_class))
    randoms.remove("random_discovery")
    print("def find_profile() - randoms: ", randoms)
